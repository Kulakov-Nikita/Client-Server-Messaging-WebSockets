...
class Emulator:
    ...
    def answer(self, message: str) -> None:
        ...
        if message_packet_type == 3:
            self.transmitter.send_device_params_data_frame()
        elif message_packet_type == 2:
            print(message)
        else:
            self.transmitter.send_error_message(error_status=1, error_comment=f"packageType = {message_packet_type} isn't supported by this device")
